...
class ActionOp {
  ...
  val conf: SparkConf = new SparkConf().setMaster("local[6]").setAppName("ActionOp")
  conf.set("spark.testing.memory", "2147480000")
  val sc = new SparkContext(conf)

  /*1.reduce 和 reduceByKey 是完全不同的, reduce 是一个 action, 并不是 Shuffled 操作
本质上 reduce 就是现在每个 partition 上求值, 最终把每个 partition 的结果再汇总*/
  ...
  def take(): Unit = {
    ...
    println(rdd.first())
    rdd.takeSample(withReplacement = false,num =3).foreach(println(_))
    println(rdd.max())
    println(rdd.min())
    println(rdd.mean())//平均值
    println(rdd.sum())
  }
}
